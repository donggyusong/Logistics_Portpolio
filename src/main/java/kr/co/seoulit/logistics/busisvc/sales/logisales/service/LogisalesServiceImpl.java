package kr.co.seoulit.logistics.busisvc.sales.logisales.service;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.TreeSet;

import kr.co.seoulit.logistics.busisvc.sales.logisales.mapper.EstimateMapper;
import kr.co.seoulit.logistics.busisvc.sales.logisales.to.ContractDetailTO;
import kr.co.seoulit.logistics.busisvc.sales.logisales.to.ContractInfoTO;
...
import kr.co.seoulit.logistics.busisvc.sales.logisales.mapper.ContractMapper;
...
public class LogisalesServiceImpl implements LogisalesService {
	...
	public ModelMap batchEstimateDetailListProcess(ArrayList<EstimateDetailTO> estimateDetailTOList,String estimateNo) {
		...
		estimateMapper.initDetailSeq(); // 상세견적 시퀀스 초기화

		//견적일련번호에 해당하는 상세견적이 있는지 확인하고 있으면 들고온다.
		...
		for(EstimateDetailTO bean : list) {// list의 사이즈가 0이기 때문에 실행 자체가 안된다
			String estimateDetailNo = bean.getEstimateDetailNo(); //상세 견적 객체의 상세견적일련번호를 담는다.
			int no = Integer.parseInt(estimateDetailNo.split("-")[1]);// 가져온 객체 자체가 없기 때문에 no는 0이다
			intSet.add(no);
		}
		...
		for (EstimateDetailTO bean : estimateDetailTOList) {
			...
			case "INSERT":
				...
				insertList.add(bean.getEstimateDetailNo()); //상세견적 일련버노를 insertList에 추가
				break;
		...
		resultMap.put("INSERT", insertList); //여기에 상세견적 일련번호를 담은 ArrayList가 담겨있다.
