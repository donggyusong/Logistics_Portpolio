  ...
  searchBtn.addEventListener("click", () => {
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState == 4 && xhr.status == 200) {
        ...
        console.log("동기-xhr.responseText",txt);
        txt = JSON.parse(txt);
        console.log("동기-JSON.parse(txt)",txt);
        if (txt.gridRowJson == "") {
        ...
        console.log("동기-txt.gridRowJson",txt.gridRowJson);
        ...
        txt.gridRowJson.map((unit, idx) => {
          ...
          [].forEach.bind(unit.estimateDetailTOList)((val) => {    // [].forEach == Array.prototype.forEach
            estimateDetailList.push(val); // estDetailGrid에서 사용하기 위해 담음
              console.log("estimateDetailList",estimateDetailList);
          });
        });
      }
    }
