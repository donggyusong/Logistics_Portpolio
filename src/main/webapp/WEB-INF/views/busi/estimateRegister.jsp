...
//  ======================일괄저장======================================================================
  ...
  batchSaveButton.addEventListener("click", () => {  //일괄저장에 걸려있는  id이름에 걸린 이벤트 처리 시즈악 !
    ...
    console.log("newEstimateRowValue-견적추가 그리드 배열",newEstimateRowValue);
    console.log("selectedRows-견적상세추가 그리드 배열",selectedRows);
    ...
    newEstimateRowValue = estGridOptions.getSelectedRowData()[0];  // 일단 견적상세(estGridOptions)그리드에서 첫번째 선택된 행의 데이터를 newEstimateRowValue에 담는다. 구길이 소스: newEstimateRowValue=newEstimateRowValue[0];
      console.log("newEstimateRowValue[0]-견적추가 그리드 배열",newEstimateRowValue);
      console.log("newEstimateRowValue.estimateDetailTOList-견적상세추가 전",newEstimateRowValue.estimateDetailTOList); //당연히 undefiend가 나온다.

    newEstimateRowValue.estimateDetailTOList = selectedRows;  //견적추가 그리드에서 가져온 데이터에 견적 상세추가 그리드 값을 넣는 로직이다.
    console.log("newEstimateRowValue.estimateDetailTOList-견적상세추가 후",newEstimateRowValue.estimateDetailTOList);
    ...
    newEstimateRowValue = JSON.stringify(newEstimateRowValue);  // 받아온 값들을 JSON형태로 바꾸어준다고 생각=> 문자열로 변환
      console.log("JSON.stringify(newEstimateRowValue)",newEstimateRowValue);
    Swal.fire({
      title: "견적을 등록하시겠습니까?",
      icon: 'warning',
